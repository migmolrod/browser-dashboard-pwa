...
|--------------|---------------------------|-------------|--------|
| FR-TSK-001   | Task management           | Must Have   | Draft  |
| FR-TSK-002   | Task status lifecycle     | Must Have   | Draft  |
| FR-TSK-003   | Task priority management  | Must Have   | Draft  |
| FR-TSK-004   | Due date management       | Must Have   | Draft  |
| FR-TSK-005   | Task lists/sections       | Should Have | Draft  |
| FR-TSK-006   | Task reordering           | Should Have | Draft  |
| FR-TSK-007   | Task search and filtering | Should Have | Draft  |
| FR-TSK-008   | Calendar integration      | Should Have | Draft  |
| FR-TSK-009   | Task completion tracking  | Must Have   | Draft  |
...
- Default status is "Todo" for new tasks
...
- US-TSK-001-004: Create task with description
...
##### Scenario 2: Create task with title exceeding maximum length
...
##### Scenario 6: Create task without title
...
- Task IDs should be generated using strongly-typed identifier pattern
- Implement validation for title length and description length

**Open questions:**

- Should deleted tasks be recoverable (soft delete with trash/recycle bin)?
...
- US-TSK-002-002: Mark task as "Done"
- US-TSK-002-003: Revert completed task to "Todo"
...
##### Scenario 2: Create task with default priority
...
- Should priority be combined with due date for smart sorting?
...
- If set, due date must be a valid timestamp
...
- User can set due date via date picker
- User can choose date-only or date+time
- Due date is validated for valid format
- TaskUpdated event is published when due date changes
...
- US-TSK-004-001: Set due date for task
...
##### Scenario 1: Set due date for task (happy path)
...
- Use date picker library for consistent UX
...
- User can create a new list with unique name
- User can rename lists (except default)
- User can delete empty lists
- Deleting non-empty list prompts for action (move tasks or delete)
- User can move tasks between lists
- Task list view shows lists in sidebar or tabs
...
- US-TSK-005-002: Move task to different list
...
##### Scenario 2: Create list with duplicate name
...
##### Scenario 3: Move task to different list
...
##### Scenario 5: Delete list with tasks
...
- Consider limiting number of lists per user (e.g., max 20)
...
- Manual order persists until user changes it
...
- Visual feedback shows task being dragged
- Drop zones between tasks are indicated
- Task order is updated when dropped
- Other tasks shift to accommodate moved task
- TaskUpdated event is published with new position
...
##### Scenario 2: Move task to first position
...
- Store task order as explicit position field (integer)
...
- Search/filter is scoped to current list (or all lists)
...
- Store filter state in URL for bookmarkability (future)
...
- Task is optionally linked to created event (future)
...
- User is notified when event is created
- Task remains in task list (not deleted)
- If task has no due date, user is prompted to set date/time for event
- Task can optionally be marked as "Scheduled" (future)

**User stories:**

- US-TSK-008-001: Create event from task with due date
- US-TSK-008-002: Create event from task without due date
- US-TSK-008-003: View linked event from task (future)
...
##### Scenario 4: Create event with description
...
- Consider storing event ID on task for bidirectional linking (future)
...
- Completion timestamp is recorded when task is marked as done
...
- US-TSK-009-004: Revert completed task

---

#### Scenarios

##### Scenario 1: Mark task as completed (happy path)
...
##### Scenario 5: Revert completed task to incomplete
...
- Should completed tasks be automatically archived after a certain time?
...
- **Task Management ↔ Status Lifecycle:** Creating tasks initializes with default status; status changes update task
  state
- **Task Management ↔ Priority Management:** Creating tasks initializes with default priority; priority changes are
  independent
...
- **Real-Time Updates:** Task widgets on dashboard should update in real-time via event subscriptions
...
- **Task Storage:** Use free-tier database (PostgreSQL via Railway, Supabase, or local)
...
Tasks management is on critical path for MVP (3-6 months):
...
- **Simplify Lists:** Start with flat list structure (defer hierarchical lists)
...
Tasks management aligns with chosen stack:
